...
// L Plate Generator by J.Rodrigo ( jrodrigo.net )
...
function getParameterDefinitions() {
  return [
	...
	{ name: 'Shole', caption: '<i>Hole spacing (mm):</i>', type: 'float', initial: 12 },
	{ name: 'Dhole', caption: '<i>Hole diameter (mm):</i>', type: 'float', initial: 3.6 },
	{ name: 'Dcorner', caption: '<i>Corner diameter (mm):</i>', type: 'float', initial: 8.5 },
...
function drills() {
    for (var iy = 0; iy < (Y); iy++) {
		w.push( CAG.circle({center: [0,Shole*iy], radius: Dhole/2, resolution: quality}) );
	}
	for (var ix = 0; ix < (X); ix++) {
		w.push( CAG.circle({center: [Shole*ix,0], radius: Dhole/2, resolution: quality}) );
	}	
}
